            ...
            <div className="p-4 sm:p-6 md:p-8">
                <header className="mb-8">
                    ...
                    <p className="text-lg text-gray-500 dark:text-gray-400">
                        Showing items for <span className="font-semibold text-cyan-500 dark:text-cyan-400">Version {version.substring(1)}</span>. Select an item to view details.
                    </p>
                </header>
                ...
                {items.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {items.sort((a, b) => a.Name.localeCompare(b.Name)).map(item => (
                            <Link
                                key={item.Name}
                                to={`${basePath}/${version}/${encodeURIComponent(activeType)}/${encodeURIComponent(item.Name)}`}
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8">
            <header className="mb-8">
                <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-2">{title}</h1>
                <p className="text-lg text-gray-500 dark:text-gray-400">
                    Core categories for <span className="font-semibold text-cyan-500 dark:text-cyan-400">Version {version.substring(1)}</span>. Select a category to view its items.
                </p>
            </header>
            ...
            {types.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {types.sort().map(type => (
                        <Link
                            key={type}
                            to={`${basePath}/${version}/${encodeURIComponent(type)}`}
                            className="block bg-white dark:bg-gray-800 rounded-lg p-6 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-200 ease-in-out group border border-gray-200 dark:border-gray-700 hover:border-cyan-500 dark:hover:border-cyan-400"
                        >
